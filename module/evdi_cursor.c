...
static void evdi_cursor_set_gem(struct evdi_cursor *cursor, struct evdi_gem_object *obj)
{
    if (obj)
...
static inline uint32_t blend_component(uint32_t pixel, uint32_t blend, uint32_t alpha)
{
...
static inline uint32_t blend_alpha(const uint32_t pixel_val32, uint32_t blend_val32)
{
...
static int evdi_cursor_compose_pixel(char __user *buffer, int const cursor_value, int const fb_value, int cmd_offset)
{
...
int evdi_cursor_compose_and_copy(struct evdi_cursor *cursor, struct evdi_framebuffer *efb, char __user *buffer, int buf_byte_stride)
{
    ...
    if (cursor->width * cursor->height * bytespp > cursor->obj->base.size)
    {
    ...
    for (y = -h_cursor_h; y < h_cursor_h; ++y)
    {
        for (x = -h_cursor_w; x < h_cursor_w; ++x)
        {
            ...
            bool const is_pix_sane = mouse_pix_x >= 0 && mouse_pix_y >= 0 && mouse_pix_x < (int) fb->width && mouse_pix_y < (int) fb->height;

            if (!is_pix_sane)
                continue;

            cursor_pix = h_cursor_w + x + (h_cursor_h + y) * cursor->width;
            curs_val = le32_to_cpu(cursor_buffer[cursor_pix]);
            fbsrc = (int *) (efb->obj->vmapping + fb->offsets[0]);
            fb_value = *(fbsrc + ((fb->pitches[0] >> 2) * mouse_pix_y + mouse_pix_x));
            cmd_offset = (buf_byte_stride * mouse_pix_y) + (mouse_pix_x * bytespp);
            if (evdi_cursor_compose_pixel(buffer, curs_val, fb_value, cmd_offset))
            {
