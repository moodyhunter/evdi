...
static void evdi_crtc_dpms(__always_unused struct drm_crtc *crtc, __always_unused int mode)
{
    EVDI_CHECKPT();
}
...
)
{
    ...
    if (notify_dpms)
        evdi_painter_dpms_notify(evdi, crtc_state->active ? DRM_MODE_DPMS_ON : DRM_MODE_DPMS_OFF);
...
)
{
    ...
    if (state->fb)
    {
        ...
        const struct drm_clip_rect fullscreen_rect = { 0, 0, fb->width, fb->height };

        if (!old_fb && crtc)
            evdi_painter_force_full_modeset(painter);

        if (old_fb && fb->format && old_fb->format && fb->format->format != old_fb->format->format)
            evdi_painter_force_full_modeset(painter);

        if (fb != old_fb || evdi_painter_needs_full_modeset(painter))
        {
...
static void evdi_cursor_atomic_get_rect(struct drm_clip_rect *rect, struct drm_plane_state *state)
{
...
)
{
    ...
    if (plane && plane->state && plane->dev && plane->dev->dev_private)
    {
        ...
        evdi_cursor_position(evdi->cursor, &cursor_position_x, &cursor_position_y);
        evdi_cursor_move(evdi->cursor, state->crtc_x, state->crtc_y);
        cursor_position_changed = cursor_position_x != state->crtc_x || cursor_position_y != state->crtc_y;
        ...
        if (fb != old_state->fb)
        {
            if (fb != NULL)
            {
                ...
                evdi_cursor_set(evdi->cursor, efb->obj, fb->width, fb->height, 0, 0, fb->format->format, stride);
            }
...
static const struct drm_plane_helper_funcs evdi_plane_helper_funcs = { .atomic_update = evdi_plane_atomic_update,
#if KERNEL_VERSION(5, 13, 0) <= LINUX_VERSION_CODE
...
static const struct drm_plane_helper_funcs evdi_cursor_helper_funcs = { .atomic_update = evdi_cursor_atomic_update,
#if KERNEL_VERSION(5, 13, 0) <= LINUX_VERSION_CODE
                                                                        .prepare_fb = drm_gem_plane_helper_prepare_fb
#else
                                                                        .prepare_fb = drm_gem_fb_prepare_fb
...
static int evdi_crtc_init(struct drm_device *dev)
{
    ...
    primary_plane = evdi_create_plane(dev, DRM_PLANE_TYPE_PRIMARY, &evdi_plane_helper_funcs);

#if KERNEL_VERSION(5, 10, 0) <= LINUX_VERSION_CODE || defined(EL8)
    cursor_plane = evdi_create_plane(dev, DRM_PLANE_TYPE_CURSOR, &evdi_cursor_helper_funcs);
#endif
...
static const struct drm_mode_config_funcs evdi_mode_funcs = { .fb_create = evdi_fb_user_fb_create,
