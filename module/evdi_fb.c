...
struct drm_clip_rect evdi_framebuffer_sanitize_rect(const struct evdi_framebuffer *fb, const struct drm_clip_rect *dirty_rect)
{
...
static void evdi_fb_fillrect(struct fb_info *info, const struct fb_fillrect *rect)
{
    ...
    evdi_handle_damage(&efbdev->efb, rect->dx, rect->dy, rect->width, rect->height);
}
...
static void evdi_fb_copyarea(struct fb_info *info, const struct fb_copyarea *region)
{
    ...
    evdi_handle_damage(&efbdev->efb, region->dx, region->dy, region->width, region->height);
}
...
static void evdi_fb_imageblit(struct fb_info *info, const struct fb_image *image)
{
    ...
    evdi_handle_damage(&efbdev->efb, image->dx, image->dy, image->width, image->height);
}
...
static int evdi_framebuffer_init(struct drm_device *dev, struct evdi_framebuffer *efb, const struct drm_mode_fb_cmd2 *mode_cmd, struct evdi_gem_object *obj)
{
    efb->obj = obj;
...
#ifdef CONFIG_FB
static int evdifb_create(struct drm_fb_helper *helper, struct drm_fb_helper_surface_size *sizes)
{
    ...
    drm_fb_helper_fill_var(info, &efbdev->helper, sizes->fb_width, sizes->fb_height);
#endif
...
struct drm_framebuffer *evdi_fb_user_fb_create(struct drm_device *dev, struct drm_file *file, const struct drm_mode_fb_cmd2 *mode_cmd)
{
    ...
    if (size > obj->size)
    {
        DRM_ERROR("object size not sufficient for fb %d %zu %u %d %d\n", size, obj->size, mode_cmd->offsets[0], mode_cmd->pitches[0], mode_cmd->height);
        goto err_no_mem;
    }
