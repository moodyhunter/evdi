...
static int copy_primary_pixels(struct evdi_framebuffer *efb, char __user *buffer, int buf_byte_stride, int num_rects, struct drm_clip_rect *rects, int const max_x,
                               int const max_y)
{
    ...
    for (r = rects; r != rects + num_rects; ++r)
    {
        ...
        const int src_offset = fb->offsets[0] + fb->pitches[0] * r->y1 + byte_offset;
...
static void copy_cursor_pixels(struct evdi_framebuffer *efb, char __user *buffer, int buf_byte_stride, struct evdi_cursor *cursor)
{
    evdi_cursor_lock(cursor);
    if (evdi_cursor_compose_and_copy(cursor, efb, buffer, buf_byte_stride))
...
u8 *evdi_painter_get_edid_copy(struct evdi_device *evdi)
{
    ...
    if (evdi_painter_is_connected(evdi->painter) && evdi->painter->edid && evdi->painter->edid_length)
    {
...
{
    return event->event->type == DRM_EVDI_EVENT_CURSOR_SET || event->event->type == DRM_EVDI_EVENT_CURSOR_MOVE;
}

static void evdi_painter_add_event_to_pending_list(struct evdi_painter *painter, struct drm_pending_event *event)
{
    unsigned long flags;
    ...
    if (!list_empty(list))
    {
        last_event = list_last_entry(list, struct drm_pending_event, link);
    }

    if (last_event && event->event->type == last_event->event->type && is_evdi_event_squashable(event))
    {
...
static bool evdi_painter_flush_pending_events(struct evdi_painter *painter)
{
    ...
    list = &painter->pending_events;
    while ((event_to_be_sent = list_first_entry_or_null(list, struct drm_pending_event, link)))
    {
        has_space = drm_event_reserve_init_locked(painter->drm_device, painter->drm_filp, event_to_be_sent, event_to_be_sent->event) == 0;
        if (has_space)
        {
            list_del_init(&event_to_be_sent->link);
            drm_send_event_locked(painter->drm_device, event_to_be_sent);
        }
        else
            break;
    }
...
static void evdi_painter_send_event(struct evdi_painter *painter, struct drm_pending_event *event)
{
    if (!event)
    {
...
static uint32_t evdi_painter_get_gem_handle(struct evdi_painter *painter, struct evdi_gem_object *obj)
{
    uint32_t handle = 0;
    ...
    if (drm_gem_handle_create(painter->drm_filp, &obj->base, &handle))
    {
...
static struct drm_pending_event *create_cursor_set_event(struct evdi_painter *painter, struct evdi_cursor *cursor)
{
    ...
    evdi_cursor_hotpoint(cursor, &event->cursor_set.hot_x, &event->cursor_set.hot_y);
    evdi_cursor_size(cursor, &event->cursor_set.width, &event->cursor_set.height);
    ...
    event->cursor_set.buffer_handle = evdi_painter_get_gem_handle(painter, eobj);
    if (eobj)
...
{
    struct drm_pending_event *event = create_cursor_set_event(painter, cursor);

    evdi_painter_send_event(painter, event);
}

static struct drm_pending_event *create_cursor_move_event(struct evdi_cursor *cursor)
{
    ...
    evdi_cursor_lock(cursor);
    evdi_cursor_position(cursor, &event->cursor_move.x, &event->cursor_move.y);
    evdi_cursor_unlock(cursor);
...
void evdi_painter_send_cursor_move(struct evdi_painter *painter, struct evdi_cursor *cursor)
{
...
static struct drm_pending_event *create_mode_changed_event(struct drm_display_mode *current_mode, int32_t bits_per_pixel, uint32_t pixel_format)
{
...
static void evdi_painter_send_mode_changed(struct evdi_painter *painter, struct drm_display_mode *current_mode, int32_t bits_per_pixel, uint32_t pixel_format)
{
    struct drm_pending_event *event = create_mode_changed_event(current_mode, bits_per_pixel, pixel_format);
...
void evdi_painter_mark_dirty(struct evdi_device *evdi, const struct drm_clip_rect *dirty_rect)
{
...
static void evdi_send_vblank(struct drm_crtc *crtc, struct drm_pending_vblank_event *vblank)
{
    if (crtc && vblank)
    {
...
void evdi_painter_set_vblank(struct evdi_painter *painter, struct drm_crtc *crtc, struct drm_pending_vblank_event *vblank)
{
    EVDI_CHECKPT();
...
void evdi_painter_dpms_notify(struct evdi_device *evdi, int mode)
{
    ...
    switch (mode)
    {
        ...
        case DRM_MODE_DPMS_OFF: mode_str = dpms_str[mode]; break;
        default: mode_str = "unknown";
    };
    EVDI_INFO("(card%d) Notifying display power state: %s", evdi->dev_index, mode_str);
...
void evdi_painter_mode_changed_notify(struct evdi_device *evdi, struct drm_display_mode *new_mode)
{
    struct evdi_painter *painter = evdi->painter;
    ...
    EVDI_INFO("(card%d) Notifying mode changed: %dx%d@%d; bpp %d; %s", evdi->dev_index, new_mode->hdisplay, new_mode->vdisplay, drm_mode_vrefresh(new_mode),
              bits_per_pixel, buf);

    evdi_painter_send_mode_changed(painter, new_mode, bits_per_pixel, pixel_format);
...
static void evdi_add_i2c_adapter(struct evdi_device *evdi)
{
    ...
    if (!evdi->i2c_adapter)
    {
        EVDI_ERROR("(card%d) Failed to allocate for i2c adapter", evdi->dev_index);
        return;
    }
    ...
    if (result)
    {
        ...
        EVDI_ERROR("(card%d) Failed to add i2c adapter, error %d", evdi->dev_index, result);
        return;
    }

    EVDI_INFO("(card%d) Added i2c adapter bus number %d", evdi->dev_index, evdi->i2c_adapter->nr);

    result = sysfs_create_link(&evdi->conn->kdev->kobj, &evdi->i2c_adapter->dev.kobj, "ddc");

    if (result)
    {
        EVDI_ERROR("(card%d) Failed to create sysfs link, error %d", evdi->dev_index, result);
        return;
    }
}
...
static void evdi_remove_i2c_adapter(struct evdi_device *evdi)
{
    if (evdi->i2c_adapter)
    {
        EVDI_INFO("(card%d) Removing i2c adapter bus number %d", evdi->dev_index, evdi->i2c_adapter->nr);
...
static int evdi_painter_connect(struct evdi_device *evdi, void const __user *edid_data, unsigned int edid_length, uint32_t sku_area_limit, struct drm_file *file,
                                __always_unused int dev_index)
{
    struct evdi_painter *painter = evdi->painter;
    ...
    expected_edid_size = sizeof(struct edid) + new_edid->extensions * EDID_EXT_BLOCK_SIZE;
...
static int evdi_painter_disconnect(struct evdi_device *evdi, struct drm_file *file)
{
    struct evdi_painter *painter = evdi->painter;
    char buf[100];
...
int evdi_painter_connect_ioctl(struct drm_device *drm_dev, void *data, struct drm_file *file)
{
    ...
    {
        if (cmd->connected)
            ret = evdi_painter_connect(evdi, cmd->edid, cmd->edid_length, cmd->sku_area_limit, file, cmd->dev_index);
        else
...
int evdi_painter_grabpix_ioctl(struct drm_device *drm_dev, void *data, __always_unused struct drm_file *file)
{
    ...
    if (painter->was_update_requested)
    {
        EVDI_WARN("(card%d) Update ready not sent,", evdi->dev_index);
    ...
    memcpy(dirty_rects, painter->dirty_rects, painter->num_dirts * sizeof(painter->dirty_rects[0]));
    ...
    if ((unsigned int) cmd->buf_width != efb->base.width || (unsigned int) cmd->buf_height != efb->base.height)
    {
    ...
    if (copy_to_user(cmd->rects, dirty_rects, cmd->num_rects * sizeof(cmd->rects[0])))
    {
        err = -EFAULT;
        goto err_fb;
    }
    ...
    err = copy_primary_pixels(efb, cmd->buffer, cmd->buf_byte_stride, cmd->num_rects, dirty_rects, cmd->buf_width, cmd->buf_height);
    if (err == 0 && !evdi->cursor_events_enabled)
        copy_cursor_pixels(efb, cmd->buffer, cmd->buf_byte_stride, evdi->cursor);
...
int evdi_painter_request_update_ioctl(struct drm_device *drm_dev, __always_unused void *data, __always_unused struct drm_file *file)
{
    ...
    if (painter)
    {
        ...
        if (painter->was_update_requested)
        {
            EVDI_WARN("(card%d) Update was already requested - ignoring\n", evdi->dev_index);
        }
        else
        {
            if (painter->num_dirts > 0)
...
{
    struct evdi_painter *painter = container_of(work, struct evdi_painter, send_events_work.work);
...
void evdi_painter_set_scanout_buffer(struct evdi_painter *painter, struct evdi_framebuffer *newfb)
{
...
static struct drm_pending_event *create_ddcci_data_event(struct i2c_msg *msg)
{
    ...
    event->ddcci_data.buffer_length = min_t(__u16, msg->len, sizeof(event->ddcci_data.buffer));
    memcpy(event->ddcci_data.buffer, msg->buf, event->ddcci_data.buffer_length);
...
static void evdi_painter_ddcci_data(struct evdi_painter *painter, struct i2c_msg *msg)
{
    ...
    if (wait_for_completion_interruptible_timeout(&painter->ddcci_response_received, msecs_to_jiffies(DDCCI_TIMEOUT_MS)) > 0)
    {
...
int evdi_painter_ddcci_response_ioctl(struct drm_device *drm_dev, void *data, __always_unused struct drm_file *file)
{
    ...
    if (copy_from_user(painter->ddcci_buffer, cmd->buffer, painter->ddcci_buffer_length))
    {
