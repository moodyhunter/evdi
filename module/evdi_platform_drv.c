...
#define evdi_platform_drv_context_lock(ctx) mutex_lock(&ctx->lock)

#define evdi_platform_drv_context_unlock(ctx) mutex_unlock(&ctx->lock)

static int evdi_platform_drv_usb(__always_unused struct notifier_block *nb, unsigned long action, void *data)
{
...
{
    struct evdi_platform_drv_context *ctx = (struct evdi_platform_drv_context *) dev_get_drvdata(device);
    struct platform_device *pdev = NULL;
...
static struct platform_driver evdi_platform_driver = { .probe = evdi_platform_device_probe,
...
static int __init evdi_init(void)
{
    ...
    if (evdi_initial_device_count)
        return evdi_platform_add_devices(g_ctx.root_dev, evdi_initial_device_count);
