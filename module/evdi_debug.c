...
void evdi_log_process(char *buf, size_t size)
{
    ...
    if (current->group_leader)
    {
        ...
        snprintf(buf, size, "Task %d (%s) of process %d (%s)", task_pid, task_comm, (int) task_pid_nr(current->group_leader), process_comm);
    }
    else
    {
        snprintf(buf, size, "Task %d (%s)", task_pid, task_comm);
    }
}
