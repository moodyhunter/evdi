...
#define evdi_log(...)                                                                                                                                                    \
    ...
    {                                                                                                                                                                    \
        ...
        {                                                                                                                                                                \
            g_evdi_logging.function(g_evdi_logging.user_data, __VA_ARGS__);                                                                                              \
        }                                                                                                                                                                \
        else                                                                                                                                                             \
        {                                                                                                                                                                \
...
struct evdi_logging g_evdi_logging = { .function = NULL, .user_data = NULL };
...
static struct evdi_frame_buffer_node *findBuffer(evdi_handle context, int id)
{
    ...
    for (node = context->frameBuffersListHead; node != NULL; node = (struct evdi_frame_buffer_node *) node->next)
    {
...
static void addFrameBuffer(evdi_handle context, struct evdi_buffer const *frame_buffer)
{
    struct evdi_frame_buffer_node **node = NULL;

    for (node = &context->frameBuffersListHead;; node = (struct evdi_frame_buffer_node **) &(*node)->next)
    {
        if (*node)
            continue;
...
static int is_evdi_compatible(int fd)
{
    ...
    evdi_log("LibEvdi version (%d.%d.%d)", LIBEVDI_VERSION_MAJOR, LIBEVDI_VERSION_MINOR, LIBEVDI_VERSION_PATCH);

    if (do_ioctl(fd, DRM_IOCTL_VERSION, &ver, "version") != 0)
        return 0;

    evdi_log("Evdi version (%d.%d.%d)", ver.version_major, ver.version_minor, ver.version_patchlevel);

    if (ver.version_major == EVDI_MODULE_COMPATIBILITY_VERSION_MAJOR && ver.version_minor >= EVDI_MODULE_COMPATIBILITY_VERSION_MINOR)
        return 1;

    evdi_log("Doesn't match LibEvdi compatibility one (%d.%d.%d)", EVDI_MODULE_COMPATIBILITY_VERSION_MAJOR, EVDI_MODULE_COMPATIBILITY_VERSION_MINOR,
...
static int is_evdi(int fd)
{
    ...
    if (do_ioctl(fd, DRM_IOCTL_VERSION, &ver, "version") == 0 && strcmp(name, "evdi") == 0)
    {
        return 1;
    }
    return 0;
...
static int device_has_master(const char *device_file_path)
{
    ...
    while ((proc_entry = readdir(proc_dir)) != NULL)
    {
        ...
        if (d_name[0] < '0' || d_name[0] > '9' || myself == atoi(d_name))
        {
            continue;
        }
...
static int write_add_device(const char *buffer, size_t buffer_length)
{
    ...
    if (add_devices != NULL)
    {
        written = fwrite(buffer, 1, buffer_length, add_devices);
        fclose(add_devices);
    }
...
static enum evdi_device_status evdi_device_to_platform(int device, char *path)
{
    ...
    while ((fd_entry = readdir(fd_dir)) != NULL)
    {
        ...
        snprintf(path, PATH_MAX, "/sys/bus/platform/devices/%s", fd_entry->d_name);
...
void evdi_enable_cursor_events(evdi_handle handle, bool enable)
{
    ...
    evdi_log("%s cursor events on /dev/dri/card%d", (enable ? "Enabling" : "Disabling"), handle->device_index);
...
void evdi_grab_pixels(evdi_handle handle, struct evdi_rect *rects, int *num_rects)
{
    ...
    if (!destinationNode)
    {
        evdi_log("Buffer %d not found. Not grabbing.", handle->bufferToUpdate);
        *num_rects = 0;
        return;
    }
    ...
    struct drm_evdi_grabpix grab = {
        EVDI_GRABPIX_MODE_DIRTY, destinationBuffer->width, destinationBuffer->height, destinationBuffer->stride, destinationBuffer->buffer, MAX_DIRTS, kernelDirts
    };
...
void evdi_ddcci_response(evdi_handle handle, const unsigned char *buffer, const uint32_t buffer_length, const bool result)
{
    ...
    do_ioctl(handle->fd, DRM_IOCTL_EVDI_DDCCI_RESPONSE, &cmd, "ddcci_response");
}
...
static uint64_t evdi_get_dumb_offset(evdi_handle ehandle, uint32_t handle)
{
    ...
    do_ioctl(ehandle->fd, DRM_IOCTL_MODE_MAP_DUMB, &map_dumb, "DRM_MODE_MAP_DUMB");

    return map_dumb.offset;
}

static struct evdi_cursor_set to_evdi_cursor_set(evdi_handle handle, struct drm_evdi_event_cursor_set *event)
{
    ...
    if (event->enabled)
    {
        ...
        uint64_t offset = evdi_get_dumb_offset(handle, event->buffer_handle);
        void *ptr = mmap(0, size, PROT_READ, MAP_SHARED, handle->fd, offset);
...
static struct evdi_cursor_move to_evdi_cursor_move(struct drm_evdi_event_cursor_move *event)
{
...
static struct evdi_ddcci_data to_evdi_ddcci_data(struct drm_evdi_event_ddcci_data *event)
{
...
static void evdi_handle_event(evdi_handle handle, struct evdi_event_context *evtctx, struct drm_event *e)
{
    switch (e->type)
    {
        ...
        case DRM_EVDI_EVENT_DPMS:
            ...
            {
                struct drm_evdi_event_dpms *event = (struct drm_evdi_event_dpms *) e;

                evtctx->dpms_handler(event->mode, evtctx->user_data);
            }
            break;
        ...
        case DRM_EVDI_EVENT_CURSOR_SET:
            ...
            {
                struct drm_evdi_event_cursor_set *event = (struct drm_evdi_event_cursor_set *) e;
        ...
        case DRM_EVDI_EVENT_CURSOR_MOVE:
            ...
            {
                struct drm_evdi_event_cursor_move *event = (struct drm_evdi_event_cursor_move *) e;

                evtctx->cursor_move_handler(to_evdi_cursor_move(event), evtctx->user_data);
            }
            break;
        ...
        case DRM_EVDI_EVENT_DDCCI_DATA:
            ...
            {
                struct drm_evdi_event_ddcci_data *event = (struct drm_evdi_event_ddcci_data *) e;

                evtctx->ddcci_data_handler(to_evdi_ddcci_data(event), evtctx->user_data);
            }
            break;

        default: evdi_log("Warning: Unhandled event");
    }
}
